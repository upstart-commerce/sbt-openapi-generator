...
trait OpenAPIGeneratorKeys {
  ...
  lazy val openapiEnableMinimalUpdate =
    settingKey[Boolean]("Only write output files that have changed")

  lazy val openapiEnablePostProcessFile =
    settingKey[Boolean]("Enable post-processing file using environment variables")

  lazy val openapiGenerateAliasAsModel =
    settingKey[Boolean]("Generate alias to map, array as models")
  ...
  lazy val openapiInputSpec =
    settingKey[URI]("Location of the OpenAPI spec, as URL or file (required)")
  ...
  lazy val openapiReleaseNote =
    settingKey[Option[String]]("Release note, default to 'Minor update'")
