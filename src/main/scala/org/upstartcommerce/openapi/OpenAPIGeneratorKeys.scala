...
trait OpenAPIGeneratorKeys {
  ...
  def fibs(n: Int, n1: Int): Stream[Int] = Cons(() => n, () => fibs(n1, n + n1))
  ...
  def forAll[A](s: Stream[A])(p: A => Boolean): Boolean = s match {
    case Cons(h, _) if !p(h()) => false
    case Cons(_, t) => forAll(t())(p)
    case Empty => true
  }

  def foldRight[A, B](s: Stream[A])(b0: B)(f: (A, B) => B): B = s match {
    case Empty => b0
    case Cons(h, t) => f(h(), foldRight(t())(b0)(f))
  }

  def takeWhile[A](s: Stream[A])(f: A => Boolean): Stream[A] = foldRight(s)(Empty: Stream[A])((l, r) => f1(l, r)(f))

  def headOption[A](s: Stream[A]): Option[A] = foldRight(s)(None: Option[A])((l, _) => Some(l))

  val f: Int => Boolean = v => v < 10

  def f1[A](a: A, b: Stream[A])(f: A => Boolean): Stream[A] = if (f(a)) Cons(() => a, () => b) else b*/
}
